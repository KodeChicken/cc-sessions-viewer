...
describe('SidebarTopbar', () => {
  it('renders the toggle, refresh, history, stats and trash buttons', () => {
    expect(factory().findAll('.top-btn')).toHaveLength(5)
  })

  it('emits toggle-sidebar / refresh / open-history / open-stats / open-trash on the matching click', async () => {
    ...
    const [historyBtn, stats, trash] = rightIcons
  ...
  it('highlights the stats button when the stats view is open', () => {
    ...
    // 第二组按钮顺序：history / stats / trash —— stats 在索引 1。
    const stats = wrapper.findAll('.topbar-icons')[1].findAll('.top-btn')[1]
