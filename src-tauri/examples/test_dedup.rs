...
fn main() {
    ...
    let with_msgid: usize = turns.iter().flat_map(|t| &t.calls).filter(|c| c.message_id.is_some()).count();
    let unique_msgid: std::collections::HashSet<&String> = turns.iter().flat_map(|t| &t.calls).filter_map(|c| c.message_id.as_ref()).collect();
    ...
    agg.feed_session(&SessionFeed {
        agent: "claude", project_dir_name: "p", project_display: "/p",
        session_id: "s", path: &path, title: "t", last_modified: 1, message_count: 0,
        turns: &turns,
    });
