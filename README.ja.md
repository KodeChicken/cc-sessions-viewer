...
**Claude Code**、**Codex**、**Gemini CLI** のローカルセッションログを閲覧するためのネイティブデスクトップアプリ。3 つの CLI の会話履歴を 1 つのタイムラインで読み・検索・再開・ソフト削除できます。

会話の再現は徹底的に忠実 —— テキスト、thinking ブロック、ツール呼び出しと結果のペアリング、構造化 diff、埋め込みスクリーンショットまで取りこぼしません。⌘⇧F は任意のキーワードからプロジェクト・エージェントを横断して該当するユーザー発言までワンショットでジャンプ。単一セッションも複数選択での一括も Markdown / HTML に書き出せます。オリジナルの JSONL は常に読み取り専用 —— 削除はすべて共有ゴミ箱へのソフト移動で、プレビューも復元も空にもでき、`rm` は一切走りません。
