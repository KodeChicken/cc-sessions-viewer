    ...
    win32wm::WM_PAINT => {
      ...
      if subclass_input.event_loop_runner.handling_events() {
        if subclass_input.event_loop_runner.should_buffer() {
          ...
          // Win32 can synchronously dispatch a nested WM_PAINT while another window's
          // paint messages are being flushed. Defer this paint instead of panicking.
          let _ = RedrawWindow(Some(window), None, None, RDW_INTERNALPAINT);
        }
      }
