...
impl<T> EventLoopRunner<T> {
  ...
  unsafe fn call_event_handler(&self, event: Event<'_, T>) {
    self.catch_unwind(|| {
      ...
      match self.event_handler.take() {
        ...
        // A missing handler means Win32 synchronously re-entered dispatch while the
        // callback was already running, or delivered a late event during teardown.
  ...
  unsafe fn move_state_to(&self, new_runner_state: RunnerState) {
    ...
    match (
      ...
      // Ignore late Win32 messages after teardown instead of reviving the runner or panicking.
